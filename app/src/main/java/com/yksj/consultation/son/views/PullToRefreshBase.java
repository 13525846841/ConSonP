...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	public static final int HEADER_INPUT = 0x1;
	public static final int HEADER_PROGRESSBAR = 0x2;
	public static final int HEADER_NOMAL = 0x3;
	public static final int HEADER_MORE = 0x4;
	public static final int HEADER_DIFFERENT = 0x5;
	...
	private int headerType = HEADER_NOMAL;
	...
	public final void setRefreshTime(Context context){
		if (null != headerLayout) {
			headerLayout.setTime(context);
		}
	}
	...
	public final void setFooterInvisiblle(int state){
		if(null != footerLayout)
			footerLayout.setVisibility(state);
	}
	
	public final void seHeaderInvisiblle(int state){
		if(null != headerLayout)
			headerLayout.setVisibility(state);
	}
	
	public final void changleHeader(Context context,int headerType){
		removeViewAt(0);
		...
		headerLayout = new LoadingLayout(context,MODE_PULL_DOWN_TO_REFRESH, headerType,releaseLabel, pullLabel,
				refreshingLabel);
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (a.hasValue(R.styleable.PullToRefresh_headerType)) {
			headerType = a.getInteger(R.styleable.PullToRefresh_headerType, HEADER_NOMAL);
		}
		...
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			if(headerType == HEADER_DIFFERENT)
				headerLayout = new LoadingLayout(context,MODE_PULL_DOWN_TO_REFRESH, HEADER_NOMAL,releaseLabel, pullLabel,
						refreshingLabel);
			else
				headerLayout = new LoadingLayout(context,MODE_PULL_DOWN_TO_REFRESH, headerType,releaseLabel, pullLabel,
						refreshingLabel);
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			if(headerType == HEADER_DIFFERENT)
				footerLayout = new LoadingLayout(context,MODE_PULL_DOWN_TO_REFRESH, HEADER_MORE,releaseLabel, pullLabel,
						refreshingLabel);
			else
			    footerLayout = new LoadingLayout(context,MODE_PULL_UP_TO_REFRESH,headerType,releaseLabel, pullLabel, refreshingLabel);
			//footerLayout.setVisibility(View.INVISIBLE);
