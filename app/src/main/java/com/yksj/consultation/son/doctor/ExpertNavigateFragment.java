 ...
 * 搜索专家时的导航
 */
public class ExpertNavigateFragment extends Fragment implements OnClickListener {
	private LayoutInflater inflater;
	LinearLayout navLayout;//导航布局
	private PopupWindow popupWindow;
	private LinearLayout contentLayout;//PopupWindow内容布局
	private LinearLayout popupWLayout, listLayout, listLayout2,otherLayout, offListLayout;//列表布局,输入布局
	...
	//记录下每个item所处的位置
	private int proPosition = 0, cityPosition = 0,clinicPosition = 0;
	...
	public View onCreateView(LayoutInflater inflater,
		...
		view = inflater.inflate(R.layout.expert_navigation_layout, null);
...
//					    area=(position1==0?"地区":proList.get(position1).get("name"));
				...
				break;
			case R.id.navigationbar_office://科室
				...
				if (group == null)//没有数据则查询
					queryOffice();
				firstAdapter2.setmSelectedPosition(clinicPosition, null);
				firstList2.setOnItemClickListener(new OnItemClickListener() {
					@Override
					public void onItemClick(AdapterView<?> parent, View view,final int position1, long id) {
						clinicPosition = position1;
						secondAdapter2.setCurrentPosition(clinicPosition);
						firstAdapter2.setmSelectedPosition(clinicPosition, view);
						firstAdapter2.notifyDataSetChanged();
						secondAdapter2.setDatas(children.get(clinicPosition));
					}
				});
				secondList2.setOnItemClickListener(new OnItemClickListener() {
					@Override
					public void onItemClick(AdapterView<?> parent, View view,
											int position, long id) {
...
//                        shanhou();
						...
						officeId=children.get(clinicPosition).get(position).get("OFFICE_CODE");

//                        if ("全部".equals(map.get("name")))
//                            tvOffice.setText("科室");
//                        else

						tvOffice.setText(children.get(clinicPosition).get(position).get("name"));
						selectorListener.goNotifyLoadData(qCity,hospitalId, officeId);
						shanhou();
...
//				listLayout.setVisibility(View.GONE);
//				otherLayout.setVisibility(View.VISIBLE);
//				otherLayout.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,LinearLayout.LayoutParams.WRAP_CONTENT));
//				otherList.setOnItemClickListener(new OnItemClickListener() {
//					@Override
//					public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
//						Map<String, String> map = otherAdapter.datas.get(position);
//						officeId=map.get("code");
//						if ("全部".equals(map.get("name")))
//							tvOffice.setText("科室");
//						else
//							tvOffice.setText(otherAdapter.datas.get(position).get("name"));
//						selectorListener.goNotifyLoadData(qCity,hospitalId, officeId);
//						shanhou();
//					}
//				});
//				if (officeList!=null&&officeList.size()>0){
//					otherAdapter.setDatas(officeList);
//				}else {
//					queryOffice();
//				}
				break;
    ...
    private void initPopupLayout() {
        ...
        listLayout2 = (LinearLayout) view.findViewById(R.id.pop_list_layout2);//列表布局
