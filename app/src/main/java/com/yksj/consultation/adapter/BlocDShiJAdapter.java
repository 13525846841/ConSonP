...
public class BlocDShiJAdapter extends RecyclerView.Adapter<BlocDShiJAdapter.DsjViewHolder> {
    ...
    public void onBindViewHolder(DsjViewHolder holder, int position) {
        ...
        if (position==0){
            ...
            Glide.with(context).load(ImageLoader.getInstance().getDownPathUri(imgUrl)).error(R.drawable.waterfall_default).placeholder(R.drawable.waterfall_default).dontAnimate()
                    .into(holder.imgDsj1);
        }else {
            if (position%2!=0) {
