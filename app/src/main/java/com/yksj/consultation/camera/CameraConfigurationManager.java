...
package com.yksj.consultation.camera;
...
final class CameraConfigurationManager {
	  ...
	  void initFromCameraParameters(Camera camera) {
	    ...
	    /**
	     * 因为方向转成竖直后Screen与Camera关系仍然按照横向比例缩放，导致扭曲。
	     * 新建一个Point把screenResolution获取的屏幕分辨率x，y值颠倒一下，替换掉。
	     */
	    cameraResolution = screenResolution;
	   	Point screenResolutionForCamera = new Point();
	   	screenResolutionForCamera.x = cameraResolution.x;
	   	screenResolutionForCamera.y = cameraResolution.y;
	   	// preview size is always something like 480*320, other 320*480
	   	if (cameraResolution.x < cameraResolution.y) {
	   		screenResolutionForCamera.x = cameraResolution.y;
	   		screenResolutionForCamera.y = cameraResolution.x;
	   	}
