...
package org.staggered.lib.internal;
 ...
 */
public class StaggerGridView extends StaggerAbsListView {
	...
	private ArrayList<FixedViewInfo> mHeaderViewInfos = new ArrayList<StaggerGridView.FixedViewInfo>();
	private ArrayList<FixedViewInfo> mFooterViewInfos = new ArrayList<StaggerGridView.FixedViewInfo>();
	...
	public StaggerGridView(Context context, AttributeSet attrs) {
		this(context, attrs, R.attr.listViewStyle);
	}

	public StaggerGridView(Context context, AttributeSet attrs, int defStyle) {
	...
	public boolean removeHeaderView(View v) {
		if (mHeaderViewInfos.size() > 0) {
			...
			if (((StaggerHeaderViewAdapter) mAdapter).removeHeader(v)) {
	...
	public boolean removeFooterView(View v) {
		if (mFooterViewInfos.size() > 0) {
			boolean result = false;
			if (((StaggerHeaderViewAdapter) mAdapter).removeFooter(v)) {
				mDataSetObserver.onChanged();
				result = true;
			}
	...
	public void setAdapter(ListAdapter adapter) {
		...
		if (mHeaderViewInfos.size() > 0|| mFooterViewInfos.size() > 0) {
			mAdapter = new StaggerHeaderViewAdapter(mHeaderViewInfos, mFooterViewInfos, adapter);
		} else {
			mAdapter = adapter;
		}
...
/*	@Override
	...
	final int measureHeightOfChildren(int widthMeasureSpec, int startPosition, int endPosition,
		...
		final StaggerAbsListView.RecycleBin recycleBin = mRecycler;
	...
	private void setupChild(View child, int position, int y, boolean flowDown, int childrenLeft,
		...
		StaggerAbsListView.LayoutParams p = (StaggerAbsListView.LayoutParams) child.getLayoutParams();
		if (p == null) {
			p = new StaggerAbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
					ViewGroup.LayoutParams.WRAP_CONTENT, 0);
		}
		p.viewType = mAdapter.getItemViewType(position);

		if ((recycled && !p.forceAdd) || (p.recycledHeaderFooter &&
				p.viewType == StaggerAdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER)) {
			attachViewToParent(child, flowDown ? -1 : 0, p);
		} else {
			p.forceAdd = false;
			if (p.viewType == StaggerAdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER) {
	...
	private void scrollListItemsBy(int amount) {
		...
		if (amount < 0) {
			...
			while (first.getBottom() < listTop) {
				StaggerAbsListView.LayoutParams layoutParams = (LayoutParams) first.getLayoutParams();
			...
			while (last.getTop() > listBottom) {
				StaggerAbsListView.LayoutParams layoutParams = (LayoutParams) last.getLayoutParams();
				if (recycleBin.shouldRecycleViewType(layoutParams.viewType)) {
