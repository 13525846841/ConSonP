 ...
 */
public abstract class StaggerAbsListView extends StaggerAdapterView<ListAdapter> implements 
ViewTreeObserver.OnGlobalLayoutListener, ViewTreeObserver.OnTouchModeChangeListener {
	 ...
	 * Acts upon click
	 */
	private StaggerAbsListView.PerformClick mPerformClick;
	...
	public interface OnScrollListener {
		 ...
		 */
		public void onScroll(StaggerAbsListView view, int firstVisibleItem, int visibleItemCount,
				int totalItemCount);
	}

	public StaggerAbsListView(Context context) {
		super(context);
		initAbsListView();
...
//		initializeScrollbars(a);
	...
	public StaggerAbsListView(Context context, AttributeSet attrs) {
		this(context, attrs, R.attr.absListViewStyle);
	}

	public StaggerAbsListView(Context context, AttributeSet attrs, int defStyle) {
...
//					mScrollY = 0;
	...
	public boolean showContextMenuForChild(View originalView) {
		...
		if (longPressPosition >= 0) {
			...
			if (mOnItemLongClickListener != null) {
				handled = mOnItemLongClickListener.onItemLongClick(StaggerAbsListView.this, originalView,
		...
		case MotionEvent.ACTION_UP: {
			...
			case TOUCH_MODE_DONE_WAITING:
				...
				if (child != null && !child.hasFocusable()) {
					...
					final StaggerAbsListView.PerformClick performClick = mPerformClick;
			...
			case TOUCH_MODE_SCROLL:
				...
				if (childCount > 0) {
					...
					if (mFirstPosition == 0 && top >= mListPadding.top &&
						...
						velocityTracker.computeCurrentVelocity(1200, mMaximumVelocity);
	...
	public LayoutParams generateLayoutParams(AttributeSet attrs) {
		return new StaggerAbsListView.LayoutParams(getContext(), attrs);
	}

	@Override
	protected boolean checkLayoutParams(ViewGroup.LayoutParams p) {
		return p instanceof StaggerAbsListView.LayoutParams;
	}
	...
	public void reclaimViews(List<View> views) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			StaggerAbsListView.LayoutParams lp = (StaggerAbsListView.LayoutParams) child.getLayoutParams();
	 ...
	 *        in the recycler.
	 *
	 * @see android.widget.StaggerAbsListView.RecycleBin
	...
	class RecycleBin {
		...
		void addScrapView(View scrap) {
			StaggerAbsListView.LayoutParams lp = (StaggerAbsListView.LayoutParams) scrap.getLayoutParams();
			if (lp == null) {
				return;
			}
		...
		void scrapActiveViews() {
			...
			for (int i = count - 1; i >= 0; i--) {
				...
				if (victim != null) {
					int whichScrap = ((StaggerAbsListView.LayoutParams) victim.getLayoutParams()).viewType;
